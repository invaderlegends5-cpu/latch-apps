...
//     (mockAuth.getUserBySessionId as jest.Mock).mockResolvedValue({ id: 'user1', tenantId: 't1' });
...
//     expect(mockAuth.getUserBySessionId).toHaveBeenCalledWith(sessionId, expect.any(Object));
//     expect(mockAuth.revokeSession).toHaveBeenCalledWith(sessionId, expect.any(Object));
...
jest.mock('../utils/cookie.util', () => ({
  clearRefreshCookies: jest.fn((res: any) => {
    if (res.clearCookie) res.clearCookie('latch_refresh');
    if (res.clearCookie) res.clearCookie('latch_session');
  }),
}));
